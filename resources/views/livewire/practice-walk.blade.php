...
<style>
    ...
    /* Rung indicator: three rungs, current one lit; streak dots within. */
    .pw-ladder { display: flex; gap: 10px; margin-bottom: 26px; }
    .pw-rung-pip {
        width: 46px; height: 8px; border-radius: 999px;
    ...
    .pw-next { display: block; margin: 0 auto; background: linear-gradient(135deg, #9333ea, #db2777); border: none; border-radius: 999px; padding: 14px 34px; color: white; font-family: 'Fredoka One', cursive; font-size: 16px; cursor: pointer; }
...
<div class="pw-wrap">
    ...
    {{-- Ladder: rungs below current are done, current is active --}}
    <div class="pw-ladder" aria-label="Level {{ $currentRung }} of 3">
        @for ($r = 1; $r <= 3; $r++)
            <div class="pw-rung-pip {{ $r < $currentRung ? 'done' : ($r === $currentRung ? 'active' : '') }}"></div>
        @endfor
    </div>

    {{-- Streak dots within the current rung (need 3 in a row) --}}
