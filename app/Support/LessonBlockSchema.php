...
class LessonBlockSchema
{
    ...
    public const OPTIONAL = [
        ...
        'clock' => ['hour', 'minute', 'pm', 'targetH', 'targetM', 'targetPm', 'question', 'content'],
        'cuboid' => ['l', 'w', 'h', 'question', 'content'],
        'jug' => ['start', 'max', 'target', 'question', 'content'],
        'balance' => ['target', 'weights', 'question', 'content'],
        'angle' => ['start', 'question', 'content'],
        'ruler' => ['start', 'cm', 'question', 'content'],
        'solids' => ['question', 'content'],
        'symmetry' => ['shape', 'question', 'content'],
    ...
    public static function guide(): array
    {
        ...
        $meta = [
            ...
            'clock' => ['Clock', 'A draggable analog clock the child sets by dragging the hands; reads 12-hour and 24-hour time.', 'She drags the gold tips to set the time; the read-out shows 12-hour and 24-hour, and an AM/PM toggle. Optional "hour"/"minute"/"pm" set the start; "targetH"/"targetM"/"targetPm" mark the time to aim for (a ✓ shows when reached). Optional "question"/"content" frame the task.'],
            'cuboid' => ['Cuboid (volume)', 'Stacking unit cubes: change length/width/height and watch volume = l × w × h.', 'Buttons change "l"/"w"/"h"; the stack of unit cubes and the read-out (base area, height, volume) update. Optional "question"/"content".'],
            'jug' => ['Jug (capacity)', 'A graduated jug the child pours with a slider; reads millilitres and litres.', 'Slide to pour; the fill and read-out (mL and L) update. Optional "max" (default 1000), "start", "target" (a Check button + ✓), "question"/"content".'],
            'balance' => ['Balance (mass)', 'A balance scale: tap weights onto the pan to reach a target mass.', 'Tap "weights" onto the pan; the beam tilts and the read-out shows grams and kg. "target" is the mass to balance (Check button). Optional "weights" list, "question"/"content".'],
            'angle' => ['Angle', 'A draggable ray that turns to show acute/right/obtuse/straight/reflex angles as turns.', 'She drags the gold tip; the read-out gives the degrees and names the angle (¼/½/¾ turn markers shown). Optional "start" (degrees), "question"/"content".'],
            'ruler' => ['Ruler (length)', 'A ruler with a draggable end; reads cm, mm and m.', 'She drags the handle; the read-out shows the length in cm, mm and m. Optional "cm" (ruler length, default 15), "start", "question"/"content".'],
            'solids' => ['Solids', 'Tap a 3-D solid to see its faces, edges and vertices.', 'Tapping a solid shows its faces/edges/vertices and a note. Optional "question"/"content".'],
            'symmetry' => ['Lines of symmetry', 'Tap each fold line that mirrors the shape, then Check.', 'She taps the dashed candidate lines that are lines of symmetry, switches shape (square/rectangle/triangle), and Checks. Optional "shape" (starting shape), "question"/"content".'],
    ...
    public static function sampleBlocks(): array
    {
        return [
            ...
            ['type' => 'clock', 'hour' => 3, 'minute' => 45, 'pm' => true, 'targetH' => 3, 'targetM' => 45, 'targetPm' => true, 'question' => 'Set the clock to quarter to four in the afternoon (3:45), then read it in 24-hour time.'],
            ['type' => 'cuboid', 'l' => 3, 'w' => 2, 'h' => 2, 'content' => 'Volume = length x width x height = 3 x 2 x 2 = 12 cm³.'],
            ['type' => 'jug', 'max' => 1000, 'target' => 750, 'question' => 'Pour 750 mL, then read it in litres.'],
            ['type' => 'balance', 'target' => 350, 'weights' => [100, 50, 20, 10], 'question' => 'Balance the basket at 350 g.'],
            ['type' => 'angle', 'start' => 45, 'question' => 'Turn the ray to a right angle (90°).'],
            ['type' => 'ruler', 'cm' => 15, 'question' => 'Drag to 6.5 cm, then read it in mm.'],
            ['type' => 'solids', 'question' => 'Tap the triangular prism and count its faces, edges and vertices.'],
            ['type' => 'symmetry', 'shape' => 'square', 'question' => 'Tap every line of symmetry of the square.'],
